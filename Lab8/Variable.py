...
class Variable(object):
    ...
    def calculate_marginal_probability(self):
        ...
        for assignment in self.assignments:
            ...
            for parent_assignment in self._generate_parent_assignments():
                marginal_probability += self.get_probability(assignment, parent_assignment) * self._calculate_joint_probability_of_parents(parent_assignment)
            self.marginal_probabilities[self.assignments[assignment]] = marginal_probability

        # set this Node`s state to ready
        self.ready = True

    def _generate_parent_assignments(self):
        """ Generate all possible assignments of the parents """
        if not self.parents:
            return [()]
        parent_assignments = [[]]
        for parent in self.parents:
            ...
            for assignment in parent_assignments:
                for value in parent.assignments:
                    new_assignments.append(tuple(assignment) + (value,))
            parent_assignments = new_assignments
        return parent_assignments

    def _calculate_joint_probability_of_parents(self, parent_assignment):
        """ Calculate the joint probability of the given parent assignments """
        joint_probability = 1
        for i, parent in enumerate(self.parents):
            joint_probability *= parent.get_marginal_probability(parent_assignment[i])
